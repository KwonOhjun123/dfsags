...
def get_args():
    ...
    parser.add_argument('--cutmix_alpha',  type=float, default=0.1)
    ...
    parser.add_argument('--ema_decay',     type=float, default=0.999,
                        help='EMA shadow weights decay. 0이면 EMA 비활성화')
    parser.add_argument('--ema_start_epoch', type=int, default=100,
                        help='EMA를 시작할 epoch (이 epoch의 model weights에서 shadow 초기화)')
...
class EMA:
    ...
    매 optimizer step 이후 shadow = decay * shadow + (1-decay) * model.
    BN running stats(buffers)는 EMA 대신 model의 최신값으로 복사.
    """
...
def train_one_epoch(model, loader, optimizer, criterion, super_criterion, scaler, args, device, super_map, smooth_matrix, ema=None):
    model.train()
...
def main():
    ...
    # EMA는 ema_start_epoch에 도달했을 때 lazy 생성 (shadow가 학습된 weights에서 출발)
    ema = None
    if args.ema_decay > 0:
        print(f'EMA 예약 (decay={args.ema_decay}, start_epoch={args.ema_start_epoch})')
    ...
    ckpt_path = os.path.join(args.save_dir, 'last_checkpoint.pth')
    if args.resume and os.path.exists(ckpt_path):
        ...
        # resume: 체크포인트에 EMA state 있으면 lazy 생성 후 로드
        if args.ema_decay > 0 and 'ema_state_dict' in ckpt:
            ema = EMA(model, decay=args.ema_decay)
            ema.load_state_dict(ckpt['ema_state_dict'])
            print(f'EMA state 복원됨 (decay={args.ema_decay})')
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # ema_start_epoch 도달 시 lazy 생성 (학습된 weights에서 shadow 출발)
        if ema is None and args.ema_decay > 0 and epoch >= args.ema_start_epoch:
            ema = EMA(model, decay=args.ema_decay)
            print(f'★ Epoch {epoch}: EMA 시작 (shadow ← current model, decay={args.ema_decay})')

        train_loss, train_acc, grad_norm = train_one_epoch(model, train_loader, optimizer, criterion, super_criterion, scaler, args, device, super_map, smooth_matrix, ema=ema)
        # EMA가 켜져 있으면 shadow model로 평가 (best/save 기준도 EMA)
        eval_model = ema.shadow if ema is not None else model
        val_loss, val_acc, val_super_acc  = evaluate(eval_model, val_loader, criterion, device)
        val_top5_acc, per_super_acc       = evaluate_detailed(eval_model, val_loader, device)
        ...
        if val_acc > best_acc:
            best_acc = val_acc
        if val_super_acc > best_super_acc:
            best_super_acc = val_super_acc
            # test.py가 로드할 best 체크포인트는 EMA weights (EMA 활성 시)
